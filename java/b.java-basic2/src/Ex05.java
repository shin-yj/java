...
//			al.add("세 번째 데이터.");
//			al.add("네 번째 데이터.");
//			al.add("네 번째 데이터."); // 데이터의 중복을 허용
//			al.add(1, "삽입된 데이터.");//1번째 위치에 삽입 // 위치 데이터 놓으면! 위치에 놓는당		
...
//			String strToRemove = (String)al.get(0);// get으로 데이터 불러온다.
...
////			//4. HashSet // set은 중복 허용 x -> 같은 데이터 저장 불가, 순서에 입각해서 데이터 사용 불가.
...
//			System.out.println();	
			...
			//5. HashMap (Hashtable)
			//HashMap<String, String> al = new HashMap<>();// hash map이나 table이나 둘 중 하나 쓰면 댄다
//			Hashtable<String, String> al = new Hashtable<>();//
//			al.put("1", "첫 번째 데이터."); // put : 데이터 추가
//			al.put("2", "두 번째 데이터.");
//			al.put("3", "세 번째 데이터.");
//			al.put("4", "네 번째 데이터.");
//			al.put("5", "네 번째 데이터.");//삽입 성공 (중복 허용)
//			al.put("5", "다섯 번째 데이터.");//삽입 실패 (overwrite) -> 나중에 쓰여지는게 최종으로 저장
//			
//			System.out.println("TEST : " + al.get("5")); // 순서번호가 아니라 key값!
//			
//			Set<String> keys = al.keySet();//keySet : 키 목록 반환 (Set 형식으로) (key는 set으로 관리)
...
//			 //6. Properties  -> key-value 스타일은 문자만 받으므로 generic(<>)안해도 된다.
...
////				System.out.println(stack.peek());	//읽고 목록에 유지 -> 데이터를 지우지 않음
//			}	
//		
